...
def read_jsonl():
  ...
  file_name_str=r"C:\Users\Morning\Desktop\hiwi\gpt_score\twitter_spider\2022-1-1_2022-12-31_without_profile_twitter_labels.jsonl"
  file_name = str(file_name_str).split("\\")[-1].replace(".jsonl", "")
  with open("%s_twitter_hierarchy_labels.jsonl" % file_name,
            'w',
            encoding='utf-8') as f:
    pass
...
def count_elements(lst):
  ...
  for element in lst:
    if element in count_dict:
      ...
      count_dict[element] = 1
  return count_dict
def return_label_analyse():

  import json
  # merged_dict = merge_dictionaries([hier, hier4, hier3, hier2])
  content_list=[]

  with open(r"C:\Users\Morning\Desktop\hiwi\gpt_score\twitter_spider\2022-1-1_2022-12-31_without_profile_twitter_labels_twitter_hierarchy_labels.jsonl", 'r', encoding='utf-8') as file:
    parent_node_sum=[]
    for line in file:
          # 解析每一行的JSON内容
          json_obj = json.loads(line)
