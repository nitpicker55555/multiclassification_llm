...
def run_process(py_file,folder_path,forget_old):
    ...
    for file in tqdm(task_list, desc=py_file.replace(".py","")):
        if forget_old==True:
            command = ["python", py_file, "--file_path", file,"--forget_old"]
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--forget_old',  action='store_true', help='forget_old') #只是获取文件名
    args = parser.parse_args()

    run_process(args.py_file, args.folder,args.forget_old)
